...
/* CT-4D-CNC-FIELD1 INDUSTRIAL 4D FIELD VISUAL BLEND
   Adds a low-risk CNC coordinate/time-field layer. It is visual-only, pointer-events none, and keeps CTA authority untouched. */
(function(){
  const FIELD_VERSION = 'CT-4D-CNC-FIELD1 industrial 4D visual blend';
  function apply4dField(){
    document.body.classList.add('dkt-4d-field-lock');
    document.body.dataset.dkt4dField = FIELD_VERSION;
    document.querySelectorAll('.ct-fix-final,.m-hero,.m-title,.m-flow-card,.full-detail-modal,.desktop-detail-modal').forEach((el)=>{
      el.dataset.dkt4dField = 'cnc-time-coordinate';
    });
    localStorage.setItem('DKT_4D_FIELD_VERSION', FIELD_VERSION);
  }
  const prevRender4dField = render;
  render = function(){
    prevRender4dField();
    apply4dField();
  };
  window.dkt4dFieldHealth = function(){
    return {
      version:FIELD_VERSION,
      route:current(),
      enabled:document.body.classList.contains('dkt-4d-field-lock'),
      targets:document.querySelectorAll('[data-dkt4d-field="cnc-time-coordinate"]').length,
      ctaAtom:document.body.classList.contains('cta-atom-lock'),
      ctaFull:document.body.classList.contains('cta-full-weight-lock')
    };
  };
  try{ render(); }catch(e){ console.error('CT-4D-CNC-FIELD render failed', e); }
})();
