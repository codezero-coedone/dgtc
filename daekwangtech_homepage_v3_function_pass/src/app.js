...
(function(){
  ...
  desktopScreen = function(routeKey){
    const r=routes[routeKey]||routes.home;
    const isHomeExact = routeKey === 'home';
    const exactImage = `<img class="exact-screen" src="./public/screens/${routeImgFinal[routeKey]||r.img}" alt="${esc(r.label)} exact screen" />`;
    return `<main class="screen-wrap company-only-screen ct-fix-final ${isHomeExact?'home-reference-main':''} route-${routeKey}" data-route="${routeKey}">
      ${isHomeExact ? `<div class="home-reference-frame">${exactImage}</div><footer class="home-reference-footer">CNC 자동선반 정밀가공 회사소개</footer>` : exactImage}
      ${isHomeExact ? '' : cleanHeader(routeKey)}
      ${routeKey==='products'?productRepairOverlay():''}
      ${isHomeExact ? '' : cleanBottomPanel(routeKey)}
...
(function(){
  ...
  desktopScreen=function(routeKey){ return routeKey === 'home' ? prevDesktop(routeKey) : prevDesktop(routeKey).replace('</main>', `${proofRailDesktop(routeKey)}</main>`); };
