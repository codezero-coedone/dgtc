...
(function(){
  ...
  desktopScreen = function(routeKey){
    const html = prevDesktopScreenNoBlue(routeKey);
    const harden = `
      <div class="screen-mask ct-inquiry-out-mask" aria-hidden="true"></div>
      <button type="button" class="screen-mask ct-noop-click-shield" aria-hidden="true" tabindex="-1"></button>
    `;
    return html.replace('</main>', `${harden}</main>`);
  };
  const prevRenderNoBlue = render;
  render = function(){
    prevRenderNoBlue();
    document.body.classList.add('ct-noblue-bottom-lock');
  };
  try{ render(); }catch(e){ console.error('CT-NOBLUE-BOTTOM render failed', e); }
})();

/* CT-SSS1~8 FINAL UPGRADE LOCK
   Axis: DAEKWANG company-introduction, no public inquiry. 4D proof loop + clean B2B cards + mobile safe UX.
*/
(function(){
  const SSS_MARK='CT-SSS1~8 FINAL UPGRADE LOCK';
  const proofSteps=[
    {t:'도면 검토',b:'도면·소재·가공성 기준을 먼저 확인합니다.', target:'fields'},
    {t:'공정 설계',b:'자동선반 가공 순서와 검사 기준을 정리합니다.', target:'facilities'},
    {t:'정밀 가공',b:'CNC 자동선반 기반으로 반복 생산 안정성을 확보합니다.', target:'products'},
    {t:'측정/검사',b:'핵심 치수와 외관 상태를 출하 전 확인합니다.', target:'quality'},
    {t:'세척/포장',b:'후공정 품질을 유지해 납품 안정성을 높입니다.', target:'facilities'},
    {t:'출하 관리',b:'제품별 품질 기준과 출하 이력을 관리합니다.', target:'quality'},
  ];
  const pageMeta={
    home:{eyebrow:'DAE KWANG TECH',title:'CNC 자동선반 기반 정밀가공 회사소개',body:'대광테크는 김해 한림면 기반의 정밀 금속부품 가공 역량, 제품사례, 설비환경, 품질관리 흐름을 한 화면에서 확인할 수 있게 정리했습니다.',cta:'회사정보 보기',target:'company',img:'./public/screens/home.jpg'},
    company:{eyebrow:'COMPANY PROFILE',title:'정밀함과 납기 기준을 함께 보는 회사정보',body:'회사 개요, 사업분야, 운영 기준, 품질 원칙을 회사소개형 구조로 확인합니다.',cta:'가공분야 보기',target:'fields',img:'./public/screens/company.jpg'},
    fields:{eyebrow:'MACHINING FIELDS',title:'자동차·유압·전자·정밀 양산가공 대응',body:'소재와 형상 기준에 따라 공정 검토, 가공, 검사 흐름을 분리해 보여줍니다.',cta:'제품사례 보기',target:'products',img:'./public/screens/fields.jpg'},
    products:{eyebrow:'PRODUCT CASE',title:'제품·가공사례 중심의 증거 화면',body:'제품 사진, 소재, 공정, 공차, 적용 분야를 카드와 상세로 연결했습니다.',cta:'설비현황 보기',target:'facilities',img:'./public/screens/products.jpg'},
    facilities:{eyebrow:'EQUIPMENT FLOW',title:'자동선반 기반 정밀 가공 환경',body:'가공·검사·세척·포장 설비를 공정 흐름과 연결해 확인합니다.',cta:'품질관리 보기',target:'quality',img:'./public/screens/facilities.jpg'},
    quality:{eyebrow:'QUALITY LOOP',title:'검사 기준과 품질 지표를 함께 확인',body:'검사 프로세스, KPI, 측정 장비 기준을 품질관리 화면에서 확인합니다.',cta:'회사정보 보기',target:'company',img:'./public/screens/quality.jpg'}
  };
  function payload(kind,id){
    const d=cms();
    if(kind==='flow'){
      const i=Number(id)||0; const f=proofSteps[i]||proofSteps[0];
      return {title:`4D Proof Loop · ${f.t}`, body:f.b, img:'./public/screens/home.jpg', chips:['4D','공정흐름','품질증거'], primary:`${routes[f.target]?.label||'관련'} 보기`, target:f.target};
    }
    if(kind==='product'){
      const p=d.products.find(x=>x.id===id)||d.products[0];
      return {title:p?.title||'제품 사례', body:p?`${p.material} · ${p.process} · ${p.tolerance}. ${p.description||'제품별 가공 기준을 확인합니다.'}`:'제품 사례를 확인합니다.', img:p?.image||'./public/products/product-01.jpg', chips:[p?.category,p?.material,p?.status].filter(Boolean), primary:'설비현황 보기', target:'facilities'};
    }
    if(kind==='facility'){
      const f=d.facilities.find(x=>x.id===id)||d.facilities[0];
      return {title:f?.name||'설비현황', body:f?`${f.spec}. 상태: ${f.status}. 공정과 설비 연결을 확인합니다.`:'설비 운영 흐름을 확인합니다.', img:f?.image||'./public/screens/facilities.jpg', chips:[f?.type,f?.status,'설비'].filter(Boolean), primary:'품질관리 보기', target:'quality'};
    }
    if(kind==='quality'){
      const q=d.quality.find(x=>x.id===id)||d.quality[0];
      return {title:q?.label||'품질관리', body:q?`기준값 ${q.value}. ${q.note||'품질 기준으로 관리합니다.'}`:'품질 기준을 확인합니다.', img:'./public/screens/quality.jpg', chips:['품질',q?.value,q?.note].filter(Boolean), primary:'설비현황 보기', target:'facilities'};
    }
    if(kind==='field'){
      const f=productCards.find(x=>x[0]===id)||productCards[0];
      return {title:`${f[0]} 가공`, body:`${f[1]}. 제품 사례와 공정 흐름을 함께 확인합니다.`, img:f[2], chips:['가공분야','소재/형상','반복생산'], primary:'제품사례 보기', target:'products'};
    }
    const meta=pageMeta[kind]||pageMeta[current()]||pageMeta.home;
    return {title:meta.title, body:meta.body, img:meta.img, chips:[meta.eyebrow,'회사소개형','B2B'], primary:meta.cta, target:meta.target};
  }
  function openSafe(kind,id){ localStorage.setItem('DKT_MOBILE_DETAIL', JSON.stringify(payload(kind,id))); render(); }
  window.mobileOpenDetail=(kind,id)=>openSafe(kind,id);
  window.mobileOpenVisual=(kind,id)=>openSafe(kind,id);
  window.openMobileUxDetail=(data={})=>{ localStorage.setItem('DKT_MOBILE_DETAIL', JSON.stringify({chips:['상세','확인'], primary:'관련 화면 보기', target:current(), ...data})); render(); };
  window.mobileOpenProduct=(id)=>openSafe('product',id);
  window.mobileCloseDetail=()=>{localStorage.removeItem('DKT_MOBILE_DETAIL');localStorage.removeItem('DKT_MOBILE_PRODUCT');render();};
  window.mobileCloseSheet=window.mobileCloseDetail;
  function safeSheet(){
    const raw=localStorage.getItem('DKT_MOBILE_DETAIL'); if(!raw) return '';
    let d; try{d=JSON.parse(raw)}catch{return ''}
    const chips=(d.chips||[]).filter(Boolean).slice(0,4).map(c=>`<span>${esc(c)}</span>`).join('');
    return `<aside class="mobile-safe-sheet" role="dialog" aria-label="상세 정보">
      <button class="safe-x" onclick="mobileCloseDetail()" aria-label="닫기">×</button>
      <div class="safe-grip"></div>
      <div class="safe-media"><img src="${esc(d.img||'./public/screens/home.jpg')}" alt="${esc(d.title||'상세')}"></div>
      <div class="safe-copy"><small>DETAIL</small><b>${esc(d.title||'대광테크 상세')}</b><p>${esc(d.body||'관련 정보를 확인합니다.')}</p><div class="safe-chips">${chips}</div><button onclick="mobileCloseDetail();go('${esc(d.target||'company')}')">${esc(d.primary||'관련 화면 보기')} →</button></div>
    </aside>`;
  }
  mobileDetailOverlay=safeSheet;
  function MobileTopSafe(){return `<header class="m-top upgraded sss-mobile-top"><button onclick="go('home')" aria-label="홈"><img class="m-logo" src="./public/screens/logo-angular-transparent.png" alt="DAE KWANG TECH" /></button><strong>DAE KWANG TECH</strong><button class="m-top-link" onclick="go('company')">회사정보</button></header>`}
  function ProofLoopMobile(title='4D Proof Loop'){
    return `<section class="m-card sss-proof"><div class="m-head"><b>${title}</b><span>4D</span></div><div class="m-proof-grid">${proofSteps.map((s,i)=>`<button onclick="mobileOpenDetail('flow','${i}')"><em>${String(i+1).padStart(2,'0')}</em><b>${esc(s.t)}</b><small>${esc(s.b)}</small></button>`).join('')}</div></section>`;
  }
  function HeroMobile(route='home'){
    const m=pageMeta[route]||pageMeta.home;
    return `<section class="m-hero sss-hero" onclick="mobileOpenDetail('${route}')" style="--hero:url('${m.img}')"><span>${esc(m.eyebrow)}</span><h1>${esc(m.title)}</h1><p>${esc(m.body)}</p><div class="m4d-mini">${proofSteps.slice(0,4).map((s,i)=>`<button onclick="event.stopPropagation();mobileOpenDetail('flow','${i}')"><b>${String(i+1).padStart(2,'0')}</b><span>${esc(s.t)}</span></button>`).join('')}</div></section>`;
  }
  function InlineCTA(route='home'){
    const m=pageMeta[route]||pageMeta.home;
    return `<section class="m-card sss-cta"><b>다음 확인 동선</b><p>정보 탐색만으로 회사 역량을 확인하는 흐름입니다.</p><button onclick="go('${m.target}')">${esc(m.cta)} →</button></section>`;
  }
  MobileHome=function(){const d=cms();return `${MobileTopSafe()}<main class="mobile-app sss-mobile">${HeroMobile('home')}<section class="m-card sss-kpi"><div class="m-head"><b>핵심 지표</b><span>PROOF</span></div><div class="m-kpi quality interactive"><button onclick="mobileOpenDetail('quality','q1')"><small>월 생산</small>${esc(d.companyInfo.monthlyCapacity||'120만개+')}</button><button onclick="mobileOpenDetail('facilities')"><small>설비</small>${esc(d.companyInfo.equipmentCount||'32대+')}</button><button onclick="mobileOpenDetail('quality','q3')"><small>품질</small>${esc(d.companyInfo.qualityRate||'99.2%')}</button><button onclick="mobileOpenDetail('flow','0')"><small>흐름</small>4D</button></div></section>${ProofLoopMobile('생산 운영 4D')}<section class="m-card"><div class="m-head"><b>가공분야</b><button onclick="go('fields')">전체</button></div><div class="m-feed">${productCards.slice(0,3).map(([t,desc,img])=>`<article onclick="mobileOpenDetail('field','${esc(t)}')"><img src="${img}" alt="${esc(t)}"><div><b>${esc(t)}</b><p>${esc(desc)}</p><span>분야 상세 보기 →</span></div></article>`).join('')}</div></section>${InlineCTA('home')}</main>${safeSheet()}${Bottom('home')}`;};
  MobileCompany=function(){const d=cms();const c=d.companyInfo;return `${MobileTopSafe()}<main class="mobile-app sss-mobile">${HeroMobile('company')}<section class="m-card"><div class="m-head"><b>회사 개요</b><span>PROFILE</span></div><ul class="m-info-list"><li><b>회사명</b><span>${esc(c.name)}</span></li><li><b>사업분야</b><span>${esc(c.businessArea||'CNC 자동선반 가공')}</span></li><li><b>설립연도</b><span>${esc(c.founded||'-')}</span></li><li><b>주소</b><span>${esc(c.address)}</span></li></ul></section>${ProofLoopMobile('회사 운영 흐름')}${InlineCTA('company')}</main>${safeSheet()}${Bottom('company')}`;};
  MobileFields=function(){const active=localStorage.getItem('DKT_FIELD_SEG')||'자동차부품';const [title,desc,img]=productCards.find(p=>p[0]===active)||productCards[0];return `${MobileTopSafe()}<main class="mobile-app sss-mobile">${HeroMobile('fields')}<div class="m-segments">${productCards.map(([t])=>`<button class="${active===t?'on':''}" onclick="mobileSetField('${t}')">${t.replace('부품','')}</button>`).join('')}</div><section class="m-card visual" onclick="mobileOpenDetail('field','${esc(active)}')"><img src="${img}" alt="${esc(title)}"><b>${esc(title)} 가공</b><p>${esc(desc)}. 소재와 형상 기준에 따라 공정 흐름을 분리합니다.</p><span class="m-card-link">사진/패널 상세 보기 →</span></section>${ProofLoopMobile('가공 대응 4D')}${InlineCTA('fields')}</main>${safeSheet()}${Bottom('fields')}`;};
  MobileProducts=function(){const d=cms();const active=localStorage.getItem('DKT_PRODUCT_SEG')||'전체';const items=d.products.filter(p=>active==='전체'||p.category===active);return `${MobileTopSafe()}<main class="mobile-app sss-mobile">${HeroMobile('products')}<div class="m-segments">${['전체','자동차부품','유압부품','전자부품','정밀 양산가공'].map(k=>`<button class="${active===k?'on':''}" onclick="mobileSetProductFilter('${k}')">${k==='전체'?'전체':k.replace('부품','')}</button>`).join('')}</div><section class="m-feed products">${items.map(p=>`<article onclick="mobileOpenProduct('${p.id}')"><img src="${esc(p.image)}" alt="${esc(p.title)}"><div><b>${esc(p.title)}</b><p>${esc(p.material)} · ${esc(p.process)} · ${esc(p.tolerance)}</p><span>${esc(p.status)} · 상세 보기 →</span></div></article>`).join('')}</section>${ProofLoopMobile('제품 생산 4D')}${InlineCTA('products')}</main>${safeSheet()}${Bottom('products')}`;};
  MobileFacilities=function(){const d=cms();return `${MobileTopSafe()}<main class="mobile-app sss-mobile">${HeroMobile('facilities')}<section class="m-feed facilities">${d.facilities.map(f=>`<article onclick="mobileOpenDetail('facility','${f.id}')"><img src="${esc(f.image)}" alt="${esc(f.name)}"><div><b>${esc(f.name)}</b><p>${esc(f.spec)}</p><span>${esc(f.status)} · 설비 상세 →</span></div></article>`).join('')}</section>${ProofLoopMobile('설비-공정 4D')}${InlineCTA('facilities')}</main>${safeSheet()}${Bottom('facilities')}`;};
  MobileQuality=function(){const d=cms();return `${MobileTopSafe()}<main class="mobile-app sss-mobile">${HeroMobile('quality')}<section class="m-card"><div class="m-head"><b>품질 KPI</b><span>MONTHLY</span></div><div class="m-kpi quality interactive">${d.quality.map(q=>`<button onclick="mobileOpenDetail('quality','${q.id}')"><small>${esc(q.label)}</small>${esc(q.value)}</button>`).join('')}</div></section>${ProofLoopMobile('품질 검사 4D')}${InlineCTA('quality')}</main>${safeSheet()}${Bottom('quality')}`;};
  mobileApp=function(routeKey){ if(routeKey==='company') return MobileCompany(); if(routeKey==='fields') return MobileFields(); if(routeKey==='products') return MobileProducts(); if(routeKey==='facilities') return MobileFacilities(); if(routeKey==='quality') return MobileQuality(); return MobileHome(); };
  const prevDesktop=desktopScreen;
  function proofRailDesktop(routeKey){
    return `<section class="screen-mask ct-sss-proof-rail" aria-label="4D Proof Loop"><b>4D Proof Loop</b>${proofSteps.slice(0,5).map((s,i)=>`<button onclick="openFullDesktopDetail('${routeKey}','process')"><em>${String(i+1).padStart(2,'0')}</em><span>${esc(s.t)}</span></button>`).join('')}</section>`;
  }
  desktopScreen=function(routeKey){ return prevDesktop(routeKey).replace('</main>', `${proofRailDesktop(routeKey)}</main>`); };
  const baseRender=render;
  render=function(){ const key=current(); app.innerHTML = key.startsWith('admin') ? adminApp(key) : (isMobile() ? mobileApp(key) : desktopScreen(key)); document.body.classList.add('ct-sss1-8-lock'); };
  try{ render(); }catch(e){ console.error(SSS_MARK+' render failed',e); }
})();


/* CT-0~CT-10 FINAL AUTHORITY LOCK
   active/live source, company-introduction policy, bottom finish, compact CTA, angular modal, full click normalization, 4D proof loop, B2B evidence cards, mobile safety, verify/live QA. */
(function(){
  const FINAL_MARK='CT-0~CT-10_FINAL_AUTHORITY_LOCK';
  const finalProofSteps=[
    {t:'요구 확인', b:'제품 요구사항과 기준 치수 확인'},
    {t:'도면 검토', b:'도면 분석 및 가공성 검토'},
    {t:'공정 설계', b:'자동선반 공정과 검사 기준 설계'},
    {t:'CNC 정밀 가공', b:'반복 생산 기준의 정밀 가공'},
    {t:'측정/검사', b:'치수·형상·표면 상태 확인'},
    {t:'세척·포장', b:'이물 관리와 출하 포장'},
    {t:'납품', b:'일정 기준 출하와 이력 관리'}
  ];
  const finalMeta={
    home:{label:'홈', eyebrow:'CNC 자동선반 전문 가공업체', title:'정밀함이 만드는 차이,\n그 차이가 제조 경쟁력입니다.', body:'대광테크는 고정밀 CNC 자동선반 가공을 통해 다양한 산업의 핵심 부품을 안정적으로 생산합니다.', img:'./public/screens/home.jpg', cta:'회사소개 보기', target:'company'},
    company:{label:'회사소개', eyebrow:'COMPANY PROFILE', title:'대광테크 정밀가공 역량', body:'회사 개요, 가공 분야, 설비 환경, 품질관리 기준을 한 흐름으로 확인합니다.', img:'./public/screens/company.jpg', cta:'가공분야 보기', target:'fields'},
    fields:{label:'가공분야', eyebrow:'MACHINING FIELDS', title:'산업의 요구를 완성하는 정밀 가공 분야', body:'자동차·유압·전자·정밀 양산가공 영역을 제품 사례와 공정 흐름으로 확인합니다.', img:'./public/screens/fields.jpg', cta:'제품사례 보기', target:'products'},
    products:{label:'제품·가공사례', eyebrow:'PRODUCT CASE', title:'정밀 가공 사례와 대응 범위', body:'제품 사진, 소재, 공정, 품질 포인트를 상세 패널로 확인합니다.', img:'./public/screens/products.jpg', cta:'설비현황 보기', target:'facilities'},
    facilities:{label:'설비현황', eyebrow:'EQUIPMENT', title:'자동선반 기반의 정밀 가공 환경', body:'자동선반 라인과 검사·세척·포장 공정이 연결된 생산 환경입니다.', img:'./public/screens/facilities.jpg', cta:'품질관리 보기', target:'quality'},
    quality:{label:'품질관리', eyebrow:'QUALITY', title:'출하 전 기준으로 관리하는 품질 체계', body:'치수 안정성, 검사 기록, 납기 준수, 반복 생산 품질을 기준으로 관리합니다.', img:'./public/screens/quality.jpg', cta:'회사소개 보기', target:'company'}
  };
  const routeOrder=['home','company','fields','products','facilities','quality'];
  const evidenceCards=[
    {kind:'field', title:'자동차부품', body:'구동계·샤프트류 반복 생산 대응', img:'./public/screens/products.jpg', target:'fields'},
    {kind:'field', title:'유압부품', body:'고압 라인 연결부와 카트리지류 가공', img:'./public/screens/fields.jpg', target:'fields'},
    {kind:'field', title:'전자부품', body:'커넥터·하우징 소형 정밀 부품 대응', img:'./public/screens/home.jpg', target:'fields'},
    {kind:'field', title:'정밀 양산가공', body:'반복 치수 안정성과 출하 전 검사 기준', img:'./public/screens/quality.jpg', target:'fields'},
  ];
  const productEvidence=()=>{
    const d=cms();
    return (d.products||[]).slice(0,6).map((p,i)=>({kind:'product', id:p.id, title:p.title, body:`${p.material} · ${p.process} · ${p.tolerance}`, img:p.image || './public/screens/products.jpg', target:i%2?'quality':'facilities'}));
  };
  const facilityEvidence=()=>{
    const d=cms();
    return (d.facilities||[]).slice(0,4).map(f=>({kind:'facility', id:f.id, title:f.name, body:f.spec, img:f.image || './public/screens/facilities.jpg', target:'quality'}));
  };
  const qualityEvidence=()=>{
    const d=cms();
    return (d.quality||[]).slice(0,4).map(q=>({kind:'quality', id:q.id, title:q.label, body:`${q.value} · ${q.note||'품질 기준'}`, img:'./public/screens/quality.jpg', target:'facilities'}));
  };
  function ctaLabel(route){ return finalMeta[route]?.cta || '회사정보 보기'; }
  function ctaTarget(route){ return finalMeta[route]?.target || 'company'; }
  function setFinalDetail(payload){
    const clean={title:'대광테크 상세', body:'관련 정보를 확인합니다.', img:'./public/screens/home.jpg', target:'company', chips:['DAEKWANG','DETAIL'], ...payload};
    localStorage.setItem('DKT_FINAL_DETAIL', JSON.stringify(clean));
    localStorage.removeItem('DKT_DESKTOP_DETAIL');
    render();
  }
  function getDetailPayload(kind,id,routeKey=current()){
    const d=cms();
    if(kind==='route'){
      const m=finalMeta[id]||finalMeta[routeKey]||finalMeta.home;
      return {title:m.title.replace('\n',' '), body:m.body, img:m.img, target:m.target, chips:[m.label,m.eyebrow,'회사소개형']};
    }
    if(kind==='proof'){
      const s=finalProofSteps[Number(id)||0]||finalProofSteps[0];
      return {title:`4D · ${s.t}`, body:s.b, img:(finalMeta[routeKey]||finalMeta.home).img, target: routeKey==='home'?'fields':routeKey, chips:['4D Proof Loop','제조 공정','증거 UI']};
    }
    if(kind==='product'){
      const p=(d.products||[]).find(x=>x.id===id)||d.products?.[0];
      return {title:p?.title||'제품 사례', body:p?`${p.material} · ${p.process} · ${p.tolerance}. ${p.description||'제품별 가공 기준을 확인합니다.'}`:'제품 사례를 확인합니다.', img:p?.image||'./public/screens/products.jpg', target:'facilities', chips:[p?.category,p?.material,p?.status].filter(Boolean)};
    }
    if(kind==='facility'){
      const f=(d.facilities||[]).find(x=>x.id===id)||d.facilities?.[0];
      return {title:f?.name||'설비현황', body:f?`${f.spec}. 상태: ${f.status}.`:'설비 운영 흐름을 확인합니다.', img:f?.image||'./public/screens/facilities.jpg', target:'quality', chips:[f?.type,f?.status,'설비'].filter(Boolean)};
    }
    if(kind==='quality'){
      const q=(d.quality||[]).find(x=>x.id===id)||d.quality?.[0];
      return {title:q?.label||'품질관리', body:q?`${q.value} · ${q.note||'품질 기준으로 관리합니다.'}`:'품질 기준을 확인합니다.', img:'./public/screens/quality.jpg', target:'facilities', chips:['품질 KPI',q?.value,q?.note].filter(Boolean)};
    }
    if(kind==='field'){
      const card=evidenceCards.find(x=>x.title===id)||evidenceCards[0];
      return {title:`${card.title} 가공`, body:`${card.body}. 분야별 제품 사례와 공정 흐름을 확인합니다.`, img:card.img, target:'products', chips:['가공분야','B2B 증거','정밀가공']};
    }
    const m=finalMeta[routeKey]||finalMeta.home;
    return {title:m.title.replace('\n',' '), body:m.body, img:m.img, target:m.target, chips:[m.label,'대광테크','상세']};
  }
  window.openFinalDetail=(kind,id,routeKey=current())=>setFinalDetail(getDetailPayload(kind,id,routeKey));
  window.openDesktopDetail=(routeKey,i=0)=>{
    const list=desktopDetails?.[routeKey]||[];
    const item=list[i]||{};
    setFinalDetail({title:item.title||finalMeta[routeKey]?.title||'대광테크 상세', body:item.body||finalMeta[routeKey]?.body||'관련 정보를 확인합니다.', img:item.img?`./public/screens/${item.img}`:(finalMeta[routeKey]?.img||'./public/screens/home.jpg'), target:item.target||ctaTarget(routeKey), chips:[finalMeta[routeKey]?.label,'클릭 기능','상세']});
  };
  window.openFullDesktopDetail=(routeKey,kind='route')=>setFinalDetail(getDetailPayload(kind==='process'?'proof':'route', kind==='process'?'0':routeKey, routeKey));
  window.closeDesktopDetail=()=>{localStorage.removeItem('DKT_FINAL_DETAIL');localStorage.removeItem('DKT_DESKTOP_DETAIL');render();};
  window.finalNextDetail=()=>{ const k=current(); const i=(routeOrder.indexOf(k)+1)%routeOrder.length; setFinalDetail(getDetailPayload('route',routeOrder[i],routeOrder[i])); };
  window.finalPrevDetail=()=>{ const k=current(); const i=(routeOrder.indexOf(k)-1+routeOrder.length)%routeOrder.length; setFinalDetail(getDetailPayload('route',routeOrder[i],routeOrder[i])); };
  function FinalDetailPanel(){
    const raw=localStorage.getItem('DKT_FINAL_DETAIL'); if(!raw) return '';
    let d; try{d=JSON.parse(raw)}catch{return ''}
    const chips=(d.chips||[]).filter(Boolean).slice(0,4).map(c=>`<span>${esc(c)}</span>`).join('');
    return `<aside class="final-detail-backdrop" role="dialog" aria-label="대광테크 상세 패널">
      <section class="final-detail-panel" onclick="event.stopPropagation()">
        <button class="final-close" onclick="closeDesktopDetail()" aria-label="닫기">×</button>
        <div class="final-media"><img src="${esc(d.img)}" alt="${esc(d.title)}"><button onclick="window.open('${esc(d.img)}','_blank')">이미지 크게 보기 ↗</button></div>
        <div class="final-copy"><small>DAEKWANG TECH DETAIL</small><h2>${esc(d.title)}</h2><p>${esc(d.body)}</p><div class="final-chips">${chips}</div><div class="final-actions"><button class="primary" onclick="closeDesktopDetail();go('${esc(d.target||'company')}')">관련 화면으로 이동 →</button><button onclick="finalNextDetail()">다음 항목</button><button onclick="finalPrevDetail()">이전 항목</button></div></div>
      </section>
    </aside>`;
  }
  function FinalHeader(routeKey){
    return `<header class="final-header"><button class="final-brand" onclick="go('home')"><img src="./public/screens/logo-angular-transparent.png" alt="DAE KWANG TECH"></button><nav>${routeOrder.map(k=>`<button class="${k===routeKey?'on':''}" onclick="go('${k}')">${esc(finalMeta[k].label)}</button>`).join('')}</nav><button class="final-header-cta" onclick="go('company')">회사소개 보기 →</button></header>`;
  }
  function FinalProofRail(routeKey){
    return `<section class="final-proof-rail"><b>4D Proof Loop</b>${finalProofSteps.map((s,i)=>`<button onclick="openFinalDetail('proof','${i}','${routeKey}')"><em>${String(i+1).padStart(2,'0')}</em><span>${esc(s.t)}</span></button>`).join('')}</section>`;
  }
  function FinalEvidence(routeKey){
    let cards=evidenceCards;
    if(routeKey==='products') cards=productEvidence();
    if(routeKey==='facilities') cards=facilityEvidence();
    if(routeKey==='quality') cards=qualityEvidence();
    if(routeKey==='company') cards=[...facilityEvidence().slice(0,2), ...qualityEvidence().slice(0,2)];
    return `<section class="final-evidence-grid">${cards.slice(0,4).map(c=>`<button onclick="openFinalDetail('${c.kind}','${esc(c.id||c.title)}','${routeKey}')"><img src="${esc(c.img)}" alt="${esc(c.title)}"><b>${esc(c.title)}</b><span>${esc(c.body)}</span></button>`).join('')}</section>`;
  }
  function FinalInlineCTA(routeKey){
    const m=finalMeta[routeKey]||finalMeta.home;
    return `<section class="final-inline-cta"><div><b>다음 확인 동선</b><span>정보 탐색만으로 회사 역량을 확인하는 흐름입니다.</span></div><button onclick="go('${m.target}')">${esc(m.cta)} →</button><button class="ghost" onclick="openFinalDetail('route','${routeKey}','${routeKey}')">현재 화면 상세</button></section>`;
  }
  function FinalDesktop(routeKey){
    const m=finalMeta[routeKey]||finalMeta.home;
    return `<main class="final-desktop route-${routeKey}">
      ${FinalHeader(routeKey)}
      <section class="final-hero">
        <div><span>${esc(m.eyebrow)}</span><h1>${esc(m.title).replace(/\n/g,'<br>')}</h1><p>${esc(m.body)}</p><div class="final-hero-actions"><button onclick="go('${m.target}')">${esc(m.cta)} →</button><button class="ghost" onclick="openFinalDetail('route','${routeKey}','${routeKey}')">화면 상세 보기</button></div></div>
        <button class="final-hero-img" onclick="openFinalDetail('route','${routeKey}','${routeKey}')"><img src="${esc(m.img)}" alt="${esc(m.label)}"></button>
      </section>
      ${FinalEvidence(routeKey)}
      ${FinalProofRail(routeKey)}
      ${FinalInlineCTA(routeKey)}
      ${FinalDetailPanel()}
    </main>`;
  }
  function mobileSheet(){
    const raw=localStorage.getItem('DKT_FINAL_DETAIL') || localStorage.getItem('DKT_MOBILE_DETAIL'); if(!raw) return '';
    let d; try{d=JSON.parse(raw)}catch{return ''}
    const chips=(d.chips||[]).filter(Boolean).slice(0,4).map(c=>`<span>${esc(c)}</span>`).join('');
    return `<aside class="final-mobile-sheet" role="dialog" aria-label="상세 정보"><button class="safe-x" onclick="mobileCloseDetail()">×</button><div class="safe-grip"></div><img src="${esc(d.img||'./public/screens/home.jpg')}" alt="${esc(d.title||'상세')}"><small>DETAIL</small><b>${esc(d.title||'대광테크 상세')}</b><p>${esc(d.body||'관련 정보를 확인합니다.')}</p><div class="safe-chips">${chips}</div><button onclick="mobileCloseDetail();go('${esc(d.target||'company')}')">${esc(d.primary||'관련 화면 보기')} →</button></aside>`;
  }
  window.mobileOpenDetail=(kind,id)=>{ setFinalDetail(getDetailPayload(kind,id,current())); };
  window.mobileOpenProduct=(id)=>{ setFinalDetail(getDetailPayload('product',id,current())); };
  window.mobileCloseDetail=()=>{localStorage.removeItem('DKT_FINAL_DETAIL');localStorage.removeItem('DKT_MOBILE_DETAIL');localStorage.removeItem('DKT_MOBILE_PRODUCT');render();};
  window.mobileCloseSheet=window.mobileCloseDetail;
  function FinalMobile(routeKey){
    const m=finalMeta[routeKey]||finalMeta.home;
    let cards=evidenceCards;
    if(routeKey==='products') cards=productEvidence();
    if(routeKey==='facilities') cards=facilityEvidence();
    if(routeKey==='quality') cards=qualityEvidence();
    return `<header class="final-mtop"><button onclick="go('home')"><img src="./public/screens/logo-angular-transparent.png" alt="DAE KWANG TECH"></button><strong>DAE KWANG TECH</strong><button onclick="go('company')">회사정보</button></header><main class="final-mobile route-${routeKey}">
      <section class="final-mhero" onclick="mobileOpenDetail('route','${routeKey}')"><span>${esc(m.eyebrow)}</span><h1>${esc(m.title).replace(/\n/g,'<br>')}</h1><p>${esc(m.body)}</p></section>
      <section class="final-mproof">${finalProofSteps.slice(0,5).map((s,i)=>`<button onclick="mobileOpenDetail('proof','${i}')"><em>${String(i+1).padStart(2,'0')}</em><b>${esc(s.t)}</b><small>${esc(s.b)}</small></button>`).join('')}</section>
      <section class="final-mcards">${cards.slice(0,4).map(c=>`<button onclick="mobileOpenDetail('${c.kind}','${esc(c.id||c.title)}')"><img src="${esc(c.img)}" alt="${esc(c.title)}"><b>${esc(c.title)}</b><span>${esc(c.body)}</span></button>`).join('')}</section>
      <section class="final-mcta"><button onclick="go('${m.target}')">${esc(m.cta)} →</button></section>
    </main>${mobileSheet()}${Bottom(routeKey==='company'?'home':routeKey)}`;
  }
  const oldRender=render;
  render=function(){
    const key=current();
    document.body.className = String(document.body.className||'').split(/\s+/).filter(c=>!/^ct-|qa-enhanced|click-function-lock|full-|human-copy/.test(c)).join(' ');
    document.body.classList.add('ct-final10-lock');
    app.innerHTML = key.startsWith('admin') ? adminApp(key) : (isMobile() ? FinalMobile(key) : FinalDesktop(key));
  };
  window.addEventListener('hashchange',()=>{localStorage.removeItem('DKT_FINAL_DETAIL');localStorage.removeItem('DKT_MOBILE_DETAIL');render();});
  try{render(); console.info(FINAL_MARK);}catch(e){console.error(FINAL_MARK+' render failed',e); try{oldRender();}catch(_){} }
})();

/* CT-B1~B10_FINAL_DELIVERY_LOCK
   Live/repo marker, full CTA/card/photo/4D click normalization, mobile safe sheet, bottom finish, angular modal, evidence asset restore, strict delivery QA. */
(function(){
  const B_MARK='CT-B1~B10_FINAL_DELIVERY_LOCK';
  const B_ROUTES=['home','company','fields','products','facilities','quality'];
  const B_LABEL={home:'홈',company:'회사소개',fields:'가공분야',products:'제품·가공사례',facilities:'설비현황',quality:'품질관리'};
  const B_SCREEN={home:'./public/screens/home.jpg',company:'./public/screens/company.jpg',fields:'./public/screens/fields.jpg',products:'./public/screens/products.jpg',facilities:'./public/screens/facilities.jpg',quality:'./public/screens/quality.jpg'};
  const B_PROOF=[
    {t:'요구 확인',b:'제품 요구사항, 소재, 수량, 기준 치수를 먼저 정리합니다.',target:'company'},
    {t:'도면 검토',b:'도면과 가공성을 검토해 공정 위험을 줄입니다.',target:'fields'},
    {t:'공정 설계',b:'자동선반 공정, 치공구, 검사 기준을 연결합니다.',target:'facilities'},
    {t:'CNC 정밀 가공',b:'반복 생산 기준으로 정밀 금속부품을 가공합니다.',target:'products'},
    {t:'측정/검사',b:'치수, 형상, 표면 상태를 출하 전 기준으로 확인합니다.',target:'quality'},
    {t:'세척·포장',b:'세척과 포장 단계에서 이물과 손상을 관리합니다.',target:'facilities'},
    {t:'납품',b:'일정 기준에 맞춰 출하 이력과 품질 흐름을 관리합니다.',target:'quality'}
  ];
  const B_PAGE={
    home:{eyebrow:'CNC AUTOMATIC LATHE',title:'정밀함이 만드는 차이,\n그 차이가 제조 경쟁력입니다.',body:'대광테크는 CNC 자동선반 기반 정밀 금속부품을 안정적으로 생산하는 회사소개형 홈페이지입니다.',cta:'회사소개 보기',target:'company'},
    company:{eyebrow:'COMPANY PROFILE',title:'대광테크 정밀가공 역량',body:'회사 개요, 사업분야, 설비, 품질 기준을 한 흐름으로 확인합니다.',cta:'가공분야 보기',target:'fields'},
    fields:{eyebrow:'MACHINING FIELDS',title:'산업별 요구를 완성하는 가공분야',body:'자동차·유압·전자·정밀 양산가공 대응 범위를 제품과 공정 기준으로 확인합니다.',cta:'제품사례 보기',target:'products'},
    products:{eyebrow:'PRODUCT CASE',title:'제품·가공사례와 정밀도 증거',body:'실제 제품 사진, 소재, 공정, 품질 포인트를 상세 패널로 연결했습니다.',cta:'설비현황 보기',target:'facilities'},
    facilities:{eyebrow:'EQUIPMENT FLOW',title:'자동선반 기반의 생산 환경',body:'가공, 검사, 세척, 포장 설비가 품질 흐름과 연결된 생산 환경입니다.',cta:'품질관리 보기',target:'quality'},
    quality:{eyebrow:'QUALITY CONTROL',title:'출하 전 기준으로 관리하는 품질체계',body:'검사 기준, 품질 KPI, 납기 흐름을 기준으로 반복 생산 안정성을 확인합니다.',cta:'회사소개 보기',target:'company'}
  };
  const B_PRODUCTS=[
    {kind:'product',id:'p1',title:'정밀 샤프트 부품',body:'CNC 자동선반 기반 반복 치수 안정성을 확인하는 대표 제품입니다.',img:'./public/products/product-01.jpg',target:'products'},
    {kind:'product',id:'p2',title:'소형 정밀 가공품',body:'소형 금속부품의 외경·내경·단차 가공 사례입니다.',img:'./public/products/product-02.jpg',target:'products'},
    {kind:'product',id:'p3',title:'커넥터 하우징',body:'전자·기계 부품용 연결 하우징 가공 사례입니다.',img:'./public/products/product-03.jpg',target:'products'},
    {kind:'product',id:'p4',title:'정밀 피팅',body:'유압·공압 라인에 적용 가능한 피팅류 가공 사례입니다.',img:'./public/products/product-04.jpg',target:'products'},
    {kind:'product',id:'p5',title:'축류 부품',body:'반복 생산 기준의 축류 부품 가공 사례입니다.',img:'./public/products/product-05.jpg',target:'products'},
    {kind:'product',id:'p6',title:'양산 샘플',body:'다품종 소량부터 반복 양산까지 대응 가능한 샘플입니다.',img:'./public/products/product-06.jpg',target:'products'}
  ];
  const B_FIELDS=[
    {kind:'field',id:'자동차부품',title:'자동차부품',body:'구동계·샤프트류·정밀 체결 부품 대응',img:'./public/products/product-05.jpg',target:'fields'},
    {kind:'field',id:'유압부품',title:'유압부품',body:'고압 라인 연결부와 카트리지류 가공 대응',img:'./public/products/product-04.jpg',target:'fields'},
    {kind:'field',id:'전자부품',title:'전자부품',body:'소형 하우징과 커넥터류 정밀 가공 대응',img:'./public/products/product-03.jpg',target:'fields'},
    {kind:'field',id:'정밀 양산가공',title:'정밀 양산가공',body:'반복 치수와 출하 안정성을 중심으로 한 양산 대응',img:'./public/products/product-06.jpg',target:'fields'}
  ];
  const B_FACILITIES=[
    {kind:'facility',id:'eq1',title:'자동선반 생산 라인',body:'CNC 자동선반 기반 정밀 가공 환경을 확인합니다.',img:'./public/screens/facilities.jpg',target:'facilities'},
    {kind:'facility',id:'eq2',title:'측정·검사 환경',body:'공정 중 검사와 출하 전 검사 흐름을 확인합니다.',img:'./public/screens/quality.jpg',target:'quality'},
    {kind:'facility',id:'eq3',title:'세척·포장 흐름',body:'후공정 품질 유지와 출하 준비 과정을 확인합니다.',img:'./public/screens/facilities.jpg',target:'facilities'},
    {kind:'facility',id:'eq4',title:'생산 유연성',body:'다품종 부품에 대응하는 설비 운영 흐름을 확인합니다.',img:'./public/screens/fields.jpg',target:'fields'}
  ];
  const B_QUALITY=[
    {kind:'quality',id:'q1',title:'치수 안정성',body:'핵심 치수와 공차 기준을 출하 전 확인합니다.',img:'./public/screens/quality.jpg',target:'quality'},
    {kind:'quality',id:'q2',title:'검사 기록 관리',body:'검사 기준과 이력을 기반으로 품질 흐름을 관리합니다.',img:'./public/screens/quality.jpg',target:'quality'},
    {kind:'quality',id:'q3',title:'납기 준수 흐름',body:'생산·검사·출하 흐름을 연결해 납기를 관리합니다.',img:'./public/screens/facilities.jpg',target:'facilities'},
    {kind:'quality',id:'q4',title:'반복 생산 품질',body:'반복 생산에서도 동일한 기준을 유지하도록 관리합니다.',img:'./public/products/product-01.jpg',target:'products'}
  ];
  function BCards(route){
    if(route==='products') return B_PRODUCTS;
    if(route==='facilities') return B_FACILITIES;
    if(route==='quality') return B_QUALITY;
    if(route==='company') return [B_FIELDS[0],B_FACILITIES[0],B_QUALITY[0],B_PRODUCTS[0]];
    return B_FIELDS;
  }
  function BPayload(kind,id,route=current()){
    if(kind==='route') { const m=B_PAGE[id]||B_PAGE[route]||B_PAGE.home; return {title:m.title.replace('\n',' '),body:m.body,img:B_SCREEN[id]||B_SCREEN[route]||B_SCREEN.home,target:m.target,chips:[B_LABEL[id]||B_LABEL[route],'회사소개형','B2B']}; }
    if(kind==='proof') { const p=B_PROOF[Number(id)||0]||B_PROOF[0]; return {title:`4D Proof Loop · ${p.t}`,body:p.b,img:B_SCREEN[route]||B_SCREEN.home,target:p.target,chips:['4D','공정 증거','정밀가공']}; }
    const pool=[...B_PRODUCTS,...B_FIELDS,...B_FACILITIES,...B_QUALITY];
    const item=pool.find(x=>String(x.id)===String(id))||pool.find(x=>x.title===id)||pool[0];
    return {title:item.title,body:item.body,img:item.img,target:item.target||route,chips:[B_LABEL[route]||'상세',kind,'증거 카드']};
  }
  window.openFinalDetail=(kind,id,route=current())=>{ localStorage.setItem('DKT_B10_DETAIL',JSON.stringify(BPayload(kind,id,route))); localStorage.removeItem('DKT_FINAL_DETAIL'); localStorage.removeItem('DKT_DESKTOP_DETAIL'); render(); };
  window.closeDesktopDetail=()=>{ localStorage.removeItem('DKT_B10_DETAIL'); localStorage.removeItem('DKT_FINAL_DETAIL'); localStorage.removeItem('DKT_DESKTOP_DETAIL'); render(); };
  window.finalNextDetail=()=>{ const r=current(); const i=(B_ROUTES.indexOf(r)+1+B_ROUTES.length)%B_ROUTES.length; openFinalDetail('route',B_ROUTES[i],B_ROUTES[i]); };
  window.finalPrevDetail=()=>{ const r=current(); const i=(B_ROUTES.indexOf(r)-1+B_ROUTES.length)%B_ROUTES.length; openFinalDetail('route',B_ROUTES[i],B_ROUTES[i]); };
  function BDetail(){
    const raw=localStorage.getItem('DKT_B10_DETAIL'); if(!raw) return '';
    let d; try{d=JSON.parse(raw)}catch{return ''}
    const chips=(d.chips||[]).filter(Boolean).slice(0,4).map(x=>`<span>${esc(x)}</span>`).join('');
    return `<aside class="b10-detail-backdrop" onclick="closeDesktopDetail()" role="dialog" aria-label="대광테크 상세 패널"><section class="b10-detail-panel" onclick="event.stopPropagation()"><button class="b10-close" onclick="closeDesktopDetail()" aria-label="닫기">×</button><div class="b10-media"><img src="${esc(d.img)}" alt="${esc(d.title)}"><button onclick="window.open('${esc(d.img)}','_blank')">이미지 크게 보기 ↗</button></div><div class="b10-copy"><small>DAEKWANG TECH DETAIL</small><h2>${esc(d.title)}</h2><p>${esc(d.body)}</p><div class="b10-chips">${chips}</div><div class="b10-actions"><button class="primary" onclick="closeDesktopDetail();go('${esc(d.target||'company')}')">관련 화면으로 이동 →</button><button onclick="finalNextDetail()">다음 항목</button><button onclick="finalPrevDetail()">이전 항목</button></div></div></section></aside>`;
  }
  function BHeader(route){ return `<header class="b10-header"><button class="b10-brand" onclick="go('home')"><img src="./public/screens/logo-angular-transparent.png" alt="DAE KWANG TECH"></button><nav>${B_ROUTES.map(r=>`<button class="${r===route?'on':''}" onclick="go('${r}')">${esc(B_LABEL[r])}</button>`).join('')}</nav><button class="b10-header-cta" onclick="go('company')">회사소개 보기 →</button></header>`; }
  function BHero(route){ const m=B_PAGE[route]||B_PAGE.home; return `<section class="b10-hero"><div class="b10-hero-copy"><span>${esc(m.eyebrow)}</span><h1>${esc(m.title).replace(/\n/g,'<br>')}</h1><p>${esc(m.body)}</p><div class="b10-hero-actions"><button onclick="go('${m.target}')">${esc(m.cta)} →</button><button class="ghost" onclick="openFinalDetail('route','${route}','${route}')">화면 상세 보기</button></div></div><button class="b10-hero-img" onclick="openFinalDetail('route','${route}','${route}')"><img src="${B_SCREEN[route]||B_SCREEN.home}" alt="${esc(B_LABEL[route])}"></button></section>`; }
  function BProof(route){ return `<section class="b10-proof"><b>4D Proof Loop</b>${B_PROOF.map((p,i)=>`<button onclick="openFinalDetail('proof','${i}','${route}')"><em>${String(i+1).padStart(2,'0')}</em><span>${esc(p.t)}</span><small>${esc(p.b)}</small></button>`).join('')}</section>`; }
  function BEvidence(route){ return `<section class="b10-cards">${BCards(route).slice(0,6).map(c=>`<button onclick="openFinalDetail('${c.kind}','${esc(c.id||c.title)}','${route}')"><img src="${esc(c.img)}" alt="${esc(c.title)}"><b>${esc(c.title)}</b><span>${esc(c.body)}</span></button>`).join('')}</section>`; }
  function BInline(route){ const m=B_PAGE[route]||B_PAGE.home; return `<section class="b10-inline"><div><b>다음 확인 동선</b><span>회사정보 → 가공분야 → 제품사례 → 설비현황 → 품질관리 순서로 확인하세요.</span></div><button onclick="go('${m.target}')">${esc(m.cta)} →</button><button class="ghost" onclick="openFinalDetail('route','${route}','${route}')">현재 화면 상세</button></section>`; }
  function BDesktop(route){ return `<main class="b10-desktop route-${route}" data-marker="${B_MARK}">${BHeader(route)}${BHero(route)}${BEvidence(route)}${BProof(route)}${BInline(route)}${BDetail()}</main>`; }
  function BMobileTop(){ return `<header class="b10-mtop"><button onclick="go('home')"><img src="./public/screens/logo-angular-transparent.png" alt="DK"></button><strong>DAE KWANG TECH</strong><button onclick="go('company')">회사정보</button></header>`; }
  function BMobile(route){ const m=B_PAGE[route]||B_PAGE.home; return `${BMobileTop()}<main class="b10-mobile" data-marker="${B_MARK}"><section class="b10-mhero" onclick="openFinalDetail('route','${route}','${route}')"><span>${esc(m.eyebrow)}</span><h1>${esc(m.title).replace(/\n/g,'<br>')}</h1><p>${esc(m.body)}</p></section><section class="b10-mproof">${B_PROOF.map((p,i)=>`<button onclick="openFinalDetail('proof','${i}','${route}')"><em>${String(i+1).padStart(2,'0')}</em><b>${esc(p.t)}</b><small>${esc(p.b)}</small></button>`).join('')}</section><section class="b10-mcards">${BCards(route).slice(0,4).map(c=>`<button onclick="openFinalDetail('${c.kind}','${esc(c.id||c.title)}','${route}')"><img src="${esc(c.img)}" alt="${esc(c.title)}"><b>${esc(c.title)}</b><span>${esc(c.body)}</span></button>`).join('')}</section><section class="b10-mcta"><button onclick="go('${m.target}')">${esc(m.cta)} →</button></section></main>${BDetail()}${BBottom(route)}`; }
  function BBottom(route){ return `<nav class="b10-bottom">${['home','fields','products','facilities','quality'].map(r=>`<button class="${r===route?'on':''}" onclick="go('${r}')"><i>${r==='home'?'⌂':r==='fields'?'◆':r==='products'?'▦':r==='facilities'?'▣':'✓'}</i><span>${esc(B_LABEL[r].replace('제품·가공사례','제품').replace('설비현황','설비').replace('품질관리','품질').replace('가공분야','가공'))}</span></button>`).join('')}</nav>`; }
  const BPrevRender=render;
  render=function(){ const key=current(); document.body.classList.add('ct-b10-lock'); if(key.startsWith('admin')){ BPrevRender(); document.body.classList.add('ct-b10-lock'); return; } app.innerHTML = isMobile()?BMobile(key):BDesktop(key); };
  try{ render(); }catch(e){ console.error('CT-B1~B10 final render failed',e); BPrevRender(); }
})();
