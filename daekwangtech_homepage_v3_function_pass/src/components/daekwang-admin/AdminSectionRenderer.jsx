            ...
            <tbody>
              {state.pages.map((page) => (
                <tr key={page.id}>
                  ...
                  <td><button className="dk-icon-btn" type="button" aria-label={`${page.name} 페이지 수정`} title={`${page.name} 페이지 수정`} onClick={() => setEditor(clone(page))}><AdminIcon name="Edit" size={16} /></button></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="dk-panel dk-admin-panel"><h3>팝업 목록</h3>
        <div className="dk-control-list">
          {state.popups.map((popup) => (
            <article key={popup.id}><div><strong>{popup.title}</strong><span>{popup.placement} · {popup.startDate} ~ {popup.endDate}</span></div><StatusBadge status={popup.status} /><div className="dk-row-actions"><button type="button" aria-label={`${popup.title} 팝업 미리보기`} title={`${popup.title} 팝업 미리보기`} onClick={() => previewPopup(popup)}><AdminIcon name="Eye" size={15} /></button><button type="button" aria-label={`${popup.title} 팝업 수정`} title={`${popup.title} 팝업 수정`} onClick={() => setForm(clone(popup))}><AdminIcon name="Edit" size={15} /></button><button type="button" aria-label={`${popup.title} 팝업 노출 상태 변경`} title={`${popup.title} 팝업 노출 상태 변경`} onClick={() => actions.updatePopups((items) => items.map((item) => item.id === popup.id ? { ...item, status: item.status === "active" ? "inactive" : "active" } : item), { type: "popupStatus", title: "팝업 노출 변경", description: `${popup.title} 팝업 상태가 변경되었습니다.`, target: popup.title })}><AdminIcon name="Refresh" size={15} /></button><button className="danger" type="button" aria-label={`${popup.title} 팝업 삭제`} title={`${popup.title} 팝업 삭제`} onClick={() => deletePopup(popup)}><AdminIcon name="Trash" size={15} /></button></div></article>
          ))}
